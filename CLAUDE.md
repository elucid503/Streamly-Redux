...
The same blank-line-inside-braces rule applies to function bodies, interface declarations, hook callbacks, and all control flow blocks:
...
export function ChannelCard({ channel, isActive, onSelect }: ChannelCardProps) {
  ...
  return (
    ...
    <button className={cn("rounded-md", isActive && "ring-2")} onClick={handleClick}>
...
- Function components and hooks throughout — no class components
- Named function declarations for components; arrow functions for event handlers and callbacks
- Files under `web/src/components/ui/` are vendored shadcn primitives and are exempt from every rule in this section — do not reformat them, and re-adding a component must not be followed by a style pass
